...
// Possible states for zGHR send
enum ZerocoinSpendStatus {
    ZGHR_SPEND_OKAY = 0,                            // No error
    ZGHR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGHR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGHR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGHR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGHR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGHR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGHR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGHR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGHR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGHR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGHR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGHR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGHR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGHR_SPENT_USED_ZGHR = 14,                      // Coin has already been spend
    ZGHR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGHR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGhrBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGHROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzghrAuto) const;
    ...
    */
    ...
    CzGHRWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGHRTracker> zghrTracker;
    ...
    void setZWallet(CzGHRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zghrTracker = std::unique_ptr<CzGHRTracker>(new CzGHRTracker(strWalletFile));
    }

    CzGHRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zGHR reset */
    boost::signals2::signal<void()> NotifyzGHRReset;
