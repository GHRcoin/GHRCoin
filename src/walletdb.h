...
// Copyright (c) 2016-2018 The GHRCoin developers
...
public:
    ...
    bool WriteZGHRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGHRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGHRSeed_deprecated(uint256& seed);
    bool EraseZGHRSeed();
    bool EraseZGHRSeed_deprecated();

    bool WriteZGHRCount(const uint32_t& nCount);
    bool ReadZGHRCount(uint32_t& nCount);
