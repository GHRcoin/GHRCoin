...
#ifndef GHRCoin_ZGHRWALLET_H
#define GHRCoin_ZGHRWALLET_H
...
public:
    CzGHRWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGHR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGHR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GHRCoin_ZGHRWALLET_H
