...
CzGHRWallet::CzGHRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zghr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGHRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGHRSeed_deprecated()) {
                    LogPrintf("%s: Updated zGHR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zghr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zghr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGHRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGHRCount(nCountLastUsed);
    else if (!walletdb.ReadZGHRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGHRWallet::Lock()
{
    seedMaster = 0;
}

void CzGHRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGHRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGHR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGHRWallet::LoadMintPoolFromDB()
{
...
void CzGHRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGHRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGHRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zghrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGHRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGHR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zghrTracker which also adds to database
    pwalletMain->zghrTracker->Add(dMint, true);
...
void CzGHRWallet::SeedToZGHR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGHRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGHRWallet::GenerateDeterministicZGHR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGHRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGHRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
