...
#if defined(NDEBUG)
#error "GHRCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ughr) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zGHR spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zGHR is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zGHR spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zGHR without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zGHR spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zGHR spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zghr");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zGHR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 21230050 && nHeight > 2000000) {
		nSubsidy = 2 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZGHRStake)
{
	int64_t ret = 0;
	...
	if (nHeight <= Params().LAST_POW_BLOCK()) {
	...
	else if (nHeight <= 2500 && nHeight > Params().LAST_POW_BLOCK()) {
		ret = blockValue * 3 / 5;  //60%;
	}
	else if (nHeight <= 5000 && nHeight > 2500) {
		...
		ret = blockValue * 24 / 25;  //96%;
	}
	else if (nHeight <= 40000 && nHeight > 35000) {
		ret = blockValue * 191 / 200;  //95.5%;
	}
	else if (nHeight <= 45000 && nHeight > 40000) {
		ret = blockValue * 19 / 20;  //95%;
	}
	else if (nHeight <= 50000 && nHeight > 45000) {
		ret = blockValue * 189 / 200;  //94.5%;
	}
	else if (nHeight <= 55000 && nHeight > 50000) {
		ret = blockValue * 47 / 50;  //94%;
	}
	else if (nHeight <= 60000 && nHeight > 55000) {
		ret = blockValue * 187 / 200;  //93.5%;
	}
	else if (nHeight <= 65000 && nHeight > 60000) {
		ret = blockValue * 93 / 100;  //93%;
	}
	else if (nHeight <= 70000 && nHeight > 65000) {
		ret = blockValue * 185 / 200;  //92.5%;
	}
	else if (nHeight <= 75000 && nHeight > 70000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from GHRCoin
...
{
	RenameThread("ghrcoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZGHRMinted()
{
...
void RecalculateZGHRSpent()
{
	...
	while (true) {
		...
		//Remove spends from zGHR supply
...
bool RecalculateGHRSupply(int nHeightStart)
{
...
{
	// GHRCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZGHRSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZGHRMinted();
		RecalculateZGHRSpent();
		RecalculateGHRSupply(Params().Zerocoin_StartHeight());
	}

	//Track zGHR money supply in the block index
	if (!UpdateZGHRSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zGHR supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGhrSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zGHR spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zGHR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zGHR stake block is less than allowed start height", __func__);

	if (CZGhrStake* zGHR = dynamic_cast<CZGhrStake*>(stake)) {
		CBlockIndex* pindexFrom = zGHR->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zGHR stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zGHR stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zGHR->GetValue()));
		if (nChecksum200 != zGHR->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zGHR->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZGHR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zGHR fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ghrInputs;
		std::vector<CTxIn> zGHRInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zGHRInputs.push_back(stakeIn);
			}
			else {
				ghrInputs.push_back(stakeIn);
			}
		}
		const bool hasGHRInputs = !ghrInputs.empty();
		const bool hasZGHRInputs = !zGHRInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasGHRInputs)
					...
					for (CTxIn ghrIn : ghrInputs) {
						if (ghrIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zGHR inputs.
			if (hasZGHRInputs) {
				for (CTxIn zGhrInput : zGHRInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zGhrInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zghr");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zGHR mints and %d zGHR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// GHRCoin: We use certain sporks during IBD, so check to see if they are
