...
#include "ui_zghrcontroldialog.h"
...
std::set<std::string> ZGhrControlDialog::setSelectedMints;
std::set<CMintMeta> ZGhrControlDialog::setMints;

ZGhrControlDialog::ZGhrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGhrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGhrControlDialog::~ZGhrControlDialog()
{
    delete ui;
}

void ZGhrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGhrControlDialog::updateList()
{
...
void ZGhrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGhrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGhr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGhrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGhrControlDialog::GetSelectedMints()
{
...
void ZGhrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
