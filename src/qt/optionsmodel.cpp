...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGHRCoinAmount"))
        settings.setValue("nAnonymizeGHRCoinAmount", 1000);

    nAnonymizeGHRCoinAmount = settings.value("nAnonymizeGHRCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGHRCoinAmount"))
        SoftSetArg("-anonymizeghrcoinamount", settings.value("nAnonymizeGHRCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ghrcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGHRCoinAmount:
            return QVariant(nAnonymizeGHRCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGHRCoinAmount:
            nAnonymizeGHRCoinAmount = value.toInt();
            settings.setValue("nAnonymizeGHRCoinAmount", nAnonymizeGHRCoinAmount);
            emit anonymizeGHRCoinAmountChanged(nAnonymizeGHRCoinAmount);
            break;
        case CoinControlFeatures:
