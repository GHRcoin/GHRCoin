...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GHRCoin address (e.g. %1)").arg("GyTyyu6PQaiezrkw2ajcTJtc2ip2krta1n"));
...
{
    // return if URI is not valid or is no GHRCoin: URI
...
{
    // Convert ghrcoin:// to ghrcoin:
    //
    //    Cannot handle this later, because ghrcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ghrcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GHRCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GHRCoin.lnk
...
{
    return GetAutostartDir() / "ghrcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ghrcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GHRCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ghrcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ghrcoin app to startup item list
